...
# 获取待分析数据的分析结果
@api.route('/getTextAnalysis', methods=['POST'])
def get_text_analysis():
    data = request.get_json()
    # print(data['data_list'])
    return jsonify(EVENTMODEL.data_analysis(data['data_list']))

# 接收前端上传的文件
@api.route('/postDataFile', methods=['POST'])
def post_data_file():
    file_obj = request.files.get('file')
    txt_data = file_obj.read().decode("utf-8").splitlines()
    # print(txt_data[0])
    json_data = []
    for txt in txt_data:
        json_data.append(ast.literal_eval(txt))
    # json_data = ast.literal_eval(txt_data[0])
    # print(json_data[0])
    # print(json_data[0]['comment_id'])
    # txt_data = str(txt_data.decode('utf-8'))
    # res = FoodEEModel.get_events_from_plain_text(txt_data)
    # file_obj.save(os.path.join(CONFIG.upload_folder, file_obj.filename))
    return jsonify(EVENTMODEL.wenjian_analysis(json_data))
    # return jsonify({'json_data':'1'})

# 获取评论基本走势
@api.route('/getDailyInfo', methods=['POST'])
def get_daily_info():
...
# 获取评论分析概览
@api.route('/getAnalysisInfo', methods=['POST'])
def get_analysis_info():
    ...
    return jsonify({'code':200, 'msg':'查询成功', 'data':result})
    # return jsonify(analysisResult['statistics_result'])

def get_total_advice():
    return

@api.route('/getAnalysisInfo1', methods=['POST'])
def get_analysis_info1():
    location = request.get_json()["location"]
    variety = request.get_json()["variety"]
    start_date = request.get_json()["start_date"]
    end_date = request.get_json()["end_date"]
    ...
    if avg_heat > 4:
        result['heat_text'] = '近期{}热度很高，平均每天{:.2f}条评价。'.format(variety, avg_heat)
    elif avg_heat > 3:
        result['heat_text'] = '近期{}热度较高，平均每天{:.2f}条评价。'.format(variety, avg_heat)
    elif avg_heat > 2:
        result['heat_text'] = '近期{}热度一般，平均每天{:.2f}条评价。'.format(variety, avg_heat)
    elif avg_heat > 1:
        result['heat_text'] = '近期{}热度较低，平均每天{:.2f}条评价。'.format(variety, avg_heat)
    else:
        result['heat_text'] = '近期{}热度很低，平均每天小于1条评价。'.format(variety, avg_heat)
...
# 获取评论分析详情
@api.route('/getAnalysisDetail', methods=['POST'])
def get_analysis_detail():
    ...
    for key in analysisResult['statistics_result']['variety'][variety]['aspect']:
        ...
        #正面评价
        if len(units_dict['POS']) > 50:
            units_dict['POS'] = units_dict['POS'][:50]
        temp['positive_units'] = [x[0] for x in units_dict['POS']]

        #负面评价
        if len(units_dict['NEG']) > 50:
            units_dict['NEG'] = units_dict['NEG'][:50]
        temp['negative_units'] = [x[0] for x in units_dict['NEG']]

        #中性评价
        if len(units_dict['NEU']) > 50:
            units_dict['NEU'] = units_dict['NEU'][:50]
        temp['neutral_units'] = [x[0] for x in units_dict['NEU']]
        
        result['aspect_details'].append(temp)

    result['aspect_details'] = sorted(result['aspect_details'], key=lambda x: x["aspect"], reverse=False)
    #热词分析
    positive_list = list(zip(list(pos_dict.keys()), list(pos_dict.values())))
    negative_list = list(zip(list(neg_dict.keys()), list(neg_dict.values())))
    summary_list = list(zip(list(summary_dict.keys()), list(summary_dict.values())))

    #positive hot words
    positive_list.sort(key = lambda x :x[1], reverse = True)
    positive_list = list(filter(lambda x: x[0] != 'NONE', positive_list))
    if len(positive_list) > 5:
        positive_list = positive_list[:5]

    #negative hot words
    negative_list.sort(key = lambda x :x[1], reverse = True)
    negative_list = list(filter(lambda x: x[0] != 'NONE', negative_list))
    if len(negative_list) > 5:
        negative_list = negative_list[:5]

    #summary hot words
    summary_list.sort(key = lambda x :x[1], reverse = True)
    summary_list = list(filter(lambda x: x[0] != 'NONE', summary_list))
    if len(summary_list) > 10:
        summary_list = summary_list[:10]
    ...
    # result['hot_words'] = {
    #       "positive": [["很好", 5], ["不错", 4], ["完美", 3], ["优秀", 2], ["很棒", 1]],
    #       "negative": [["太差", 5], ["发霉", 4], ["恶心", 3], ["太慢", 2], ["难吃", 1]],
    #       "summary": [["价格实惠", 10], ["品质不错", 9], ["色热红润", 8], ["口感很好", 7], ["包装精美", 6], ["分量不足", 5], ["物流一般", 4], ["售后很差", 3], ["东西还行", 2], ["大小均匀", 1]]
    #     }
    return jsonify({'code':200, 'msg':'查询成功', 'data':result})
    # return jsonify(analysisResult['statistics_result'])

@api.route('/getAnalysisDetail1', methods=['POST'])
def get_analysis_detail1():
    location = request.get_json()["location"]
    variety = request.get_json()["variety"]
    start_date = request.get_json()["start_date"]
    end_date = request.get_json()["end_date"]
